...
function add_og_tags() {
	...
	$default_base_description = ( get_bloginfo( 'description' ) ) ? get_bloginfo( 'description' ) : esc_html__( 'Visit our website to learn more.', 'nda' );
	...
	if ( is_category() || is_tag() || is_tax() ) {
		...
		$specify        = ( is_category() ) ? esc_html__( 'categorized in', 'nda' ) : esc_html__( 'tagged with', 'nda' );
		$queried_object = get_queried_object();
		$card_url       = get_term_link( $queried_object );
		$card_type      = 'website';

		// Translators: get the term name.
		$card_long_description = sprintf( esc_html__( 'Posts %1$s %2$s.', 'nda' ), $specify, $term_name );
	...
	if ( is_search() ) {
		...
		$card_long_description = sprintf( esc_html__( 'Search results for %s.', 'nda' ), $search_term );
		$card_description      = $card_long_description;
	}
