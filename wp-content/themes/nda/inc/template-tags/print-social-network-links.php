...
function print_social_network_links() {
	...
	<ul class="flex social-icons menu">
		...
		foreach ( $social_networks as $network ) :
			...
			$network_url = get_theme_mod( 'nda_' . $network . '_link' );
			...
			if ( ! empty( $network_url ) ) :
				...
				<li class="social-icon <?php echo esc_attr( $network ); ?> mr-2">
					<a href="<?php echo esc_url( $network_url ); ?>">
						...
						printf( esc_attr__( 'Link to %s', 'nda' ), ucwords( esc_html( $network ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- XSS OK.
						?>
						</span>
					</a>
				</li><!-- .social-icon -->
